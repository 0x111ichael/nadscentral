...
export function Leaderboard() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      setLoading(true);
      // Join user_daily_stats with profiles for displayName/avatar
      const { data, error } = await supabase
        .from('user_daily_stats')
        .select(`wallet_address, current_points, total_earned, profiles:wallet_address(username, avatar_url)`)
      ...
      const entries: LeaderboardEntry[] = data.map((row: any, idx: number) => ({
        ...
        displayName: row.profiles?.username || row.wallet_address,
        nadScore: row.current_points,
        weeklyGain: 0, // TODO: calculate if available
        avatar: row.profiles?.avatar_url,
        badge: undefined // TODO: add badge logic if available
      }));
        ...
        <div className="p-6">
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <div className="text-center text-muted-foreground py-8">No leaderboard data found.</div>
